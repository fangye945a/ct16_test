...
export const MOCK_MODULE_SLOTS: IModuleSlot[] = [
  ...
  },
  {
    ...
    channelList: Array.from({ length: 8 }, (_, i) => ({
      index: i + 1,
      label: i < 4 ? `VO-${String(i + 1).padStart(2, '0')}` : `CO-${String(i - 3).padStart(2, '0')}`,
      status: 'normal' as IModuleChannel['status'],

...
export function EditModelSheet({
  ...
  const [form, setForm] = useState<EditForm>(() =>
    model ? buildInitialEditForm(model) : buildInitialEditForm({
      index: -1,
      model_name: '',
      model: '',
      api_key: '',
  ...
  useEffect(() => {
    ...
    const next = buildInitialEditForm(model);
    setForm(next);
    setInitialForm(next);
      ...
      <Sheet open={open} onOpenChange={(v) => !v && onClose()}>
        ...
        >
          <SheetHeader className="border-b border-border/40 px-6 py-5">
            <SheetTitle className="text-base">配置 {model?.model_name}</SheetTitle>
            ...
            <div className="space-y-5 px-6 py-5">
              ...
              >
                <Input
                  ...
                  aria-invalid={modelValidation?.level === 'error'}
                />
              ...
              {!isOAuth && (
                <Field
                  label="API Key"
                  hint={hasSavedAPIKey ? '已设置 API Key，留空表示不修改。' : undefined}
                >
                  <div className="relative">
                    <Input
                      ...
                      placeholder={
                        hasSavedAPIKey ? '留空保持原有 Key 不变' : '请输入 API Key'
                      }
                    />
                    <button
                      type="button"
              ...
              <Field
                label="API Base URL"
                hint={isOAuth ? '该服务商使用 OAuth 认证，无需 API Key。' : undefined}
              >
                <Input

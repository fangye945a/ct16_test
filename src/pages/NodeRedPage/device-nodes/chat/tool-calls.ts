...
function parseLegacyToolFeedbackContent(content: string): ChatToolCall[] | undefined {
...
export function parseToolCallsValue(raw: unknown): ChatToolCall[] | undefined {
  ...
  for (const item of raw) {
    ...
    if (rawFunction) {
      ...
      const argumentsText =
        typeof rawFunction.arguments === 'string' ? rawFunction.arguments : undefined;
    ...
    if (nextToolCall.id || nextToolCall.type || nextToolCall.function || nextToolCall.extraContent) {
...
export function parseToolCallsFromContent(content: string): ChatToolCall[] | undefined {

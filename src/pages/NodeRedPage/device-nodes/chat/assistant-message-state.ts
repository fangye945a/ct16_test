import { parseToolCallsFromContent, parseToolCallsValue } from './tool-calls';
import type { AssistantMessageKind, ChatMessage } from './types';
...
function normalizeAssistantMessageKind(payload: Record<string, unknown>): string | undefined {
...
function hasExplicitAssistantKindPayload(payload: Record<string, unknown>): boolean {
  return normalizeAssistantMessageKind(payload) !== undefined || payload.tool_calls !== undefined;
}
...
function parseAssistantToolCalls(
  ...
  return parseToolCallsValue(payload.tool_calls) ?? parseToolCallsFromContent(content);
}
